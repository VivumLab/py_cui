...
class PyCUI:
    ...
    def create_new_widget_set(self,
                            num_rows: int,
                            num_cols: int
        ...
        return py_cui.widget_set.WidgetSet(num_rows,
                                        num_cols,
                                        self._logger,
                                        root=self,
    ...
    def set_widget_border_characters(self,
    ...
    def _get_horizontal_neighbors(self,
                                widget: 'py_cui.widgets.Widget',
                                direction: int
                                ) -> Optional[List[int]]:
        ...
        if direction not in (py_cui.keys.KEY_LEFT_ARROW,py_cui.keys.KEY_RIGHT_ARROW):
            return None

        _, num_cols = self._grid.get_dimensions()
        row_start, col_start = widget.get_grid_cell()
        col_span, row_span = widget.get_grid_cell_spans()
        id_list = []

        if direction == py_cui.keys.KEY_LEFT_ARROW:
            scope = range(col_start - 1, -1, -1)
        else:
            scope = range(col_start + col_span, num_cols)

        widgets = self.get_widgets()
        ids = widgets.keys()

        for col in scope:
            col_items = [w for w in ids if widgets[w]._column == col and widgets[w].is_selectable()]
            sorted_items = sorted(col_items, key=lambda x: abs(row_start - widgets[x]._row))
    ...
    def _get_vertical_neighbors(self,
                                widget: "py_cui.widgets.Widget",
                                direction: int
                                ) -> Optional[List[int]]:
